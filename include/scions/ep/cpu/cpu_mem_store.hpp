...
public:
    ...
    void initializeMemory() {
        if constexpr (G.f32_size) {
            f32_container = std::make_unique<std::array<float, G.f32_size>>();
        }
        if constexpr (G.u8_size) {
            u8_container = std::make_unique<std::array<uint8_t, G.u8_size>>();
        }
        ...
        for (size_t i{}; i < G.graph_data_size; ++i) {
            ...
            case manifold::DType::UINT8:
                ...
                break;
            case manifold::DType::UINT16:
                break;
            case manifold::DType::UINT32:
                break;
            case manifold::DType::UINT64:
                break;
            case manifold::DType::INT8:
                break;
            case manifold::DType::INT16:
                break;
            case manifold::DType::INT32:
                break;
            case manifold::DType::INT64:
                break;
            case manifold::DType::F32:
                data_ptr = &f32_container->at(f32_offset);
                f32_offset += tensor.size + 1;
                break;
            case manifold::DType::F64:
                break;
            }
            if (data_ptr == nullptr) {
