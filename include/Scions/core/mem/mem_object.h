...
public:
  ...
  consteval StaticMemObject(const uint32_t size_,
  ...
  constexpr StaticMemObject(uint32_t x,
    uint32_t y,
...
public:
  ...
  constexpr MemObject()
    : bytes(0), id(UINT64_MAX), offset(0), name("bruh"), type(d_type::TYPE::F16) {}
  ...
  constexpr MemObject(const uint64_t size,
    ...
    : name(name_), bytes(size), id(id_), offset(offset_), type(type_) {
  ...
  constexpr MemObject(const StaticMemObject &object,
    const uint64_t id_,
    const uint64_t offset_)
    : name(object.name), bytes(object.total_size), id(id_), offset(offset_),
      type(object.data_type) {
    if (bytes % getDTypeSizeBytes(type) != 0) {
      throw std::logic_error("Size of input (bytes) is not divisible by "
        "size of data type. Please correct.");
    }
  }
