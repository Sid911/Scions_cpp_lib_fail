...
struct Matrix {
    ...
    [[nodiscard]] constexpr auto to_array() const {
        return Array<Type, size, Storage>(id);
    }
};
#pragma endregion
...
struct Tensor : TensorBase {
    ...
    [[nodiscard]] constexpr auto to_array() const {
        return Array<TensorBase::data_type, TensorBase::size, Storage>(id);
    }
...
const char *to_string(OpType e) {
    switch (e) {
    case ARRAY_ELM_ADD:
        return "ARRAY_ELM_ADD";
    case ARRAY_ELM_SUB:
        return "ARRAY_ELM_SUB";
    case ARRAY_ELM_MUL:
        return "ARRAY_ELM_MUL";
    case ARRAY_ELM_DIV:
        return "ARRAY_ELM_DIV";
    case RANDOM_ELM:
        return "RANDOM_ELM";
    case ARRAY_AXPY:
        return "ARRAY_AXPY";
...
struct ExpressionReflection {
    ...
    [[nodiscard]] constexpr bool isTensor() const {
...
struct Expression {
    ...
    void print() {
        ...
        for (int i = 0; i < num_inputs; ++i) {
            fmt::print("{} ", input_indices[i]);
        }
        fmt::print("\nOutputs :");
        for (int i = 0; i < num_outputs; ++i) {
            fmt::print("{} ", output_indices[i]);
        }
        fmt::print("\n");
    }
};
...
concept has_size = requires(T typ) {
    ...
    -> std::same_as<const unsigned long &>;
};
...
requires(_internal::is_array_like<T>)
{
    ...
    for (int i = 0; i < inp.size(); ++i) {
        inputs->at(i) = std::move(ExpressionReflection(inp.at(i)));
    }
...
constexpr uint32_t extractRefs(const ExpressionReflection *exp_ref,
    ...
    if (exp_ref->isTensor()) {
        ...
        auto it =
        std::ranges::find_if(data, [id = exp_ref->tensor.value().id](const TensorReflection &tr) {
            return tr.id == id;
        });
    ...
    auto it =
    std::ranges::find_if(exp_arr, [hash = expression.hash](const Expression &e) {
        return hash_value(e) == hash;
    });
    if (it == exp_arr.end()) {
...
struct fmt::formatter<manifold::Expression> {
    ...
    constexpr auto format(const manifold::Expression &exp, FormatContext &ctx) {
        ...
        for (int i = 0; i < exp.num_inputs; ++i) {
            format_to(ctx.out(), "{} ", exp.input_indices[i]);
        }
        format_to(ctx.out(), "\nOutputs: ");
        for (int i = 0; i < exp.num_outputs; ++i) {
            format_to(ctx.out(), "{} ", exp.output_indices[i]);
        }
...
int main() noexcept {
    ...
    for (const auto &data : res.data) {
        fmt::print("{}\n", data.id);
    }
    fmt::print("\nExpression hashes: \n");
    for (int i = 0; i < res.exp_len; ++i) {
