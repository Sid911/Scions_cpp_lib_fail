...
public:
    ...
    constexpr StaticMem()
        : total_size(), dimension(), data_type(d_type::F16), shape() {}

    consteval StaticMem(const uint32_t size_,
    ...
    constexpr StaticMem(uint32_t x,
                        uint32_t y,
    ...
    consteval StaticMem(std::initializer_list<uint32_t> list,
                        std::string_view name_,
                        d_type::TYPE type_ = d_type::F16)
        : name(name_),
...
public:
    ...
    constexpr MemObject()
        : bytes(0), id(UINT64_MAX), offset(UINT64_MAX), type(d_type::TYPE::F16), shape(), dimension() {}
    ...
    constexpr MemObject(const uint64_t size,
        ...
        : name(name_), bytes(size), id(id_),
          offset(offset_), type(type_), dimension(dimension_), shape(shape_) {
