...
consteval bool isOpEqual(auto ex_arr, size_t N, manifold::OpType &type) {
    return ex_arr[N].type == type;
}

inline void invalidCpuOp() {
    throw std::runtime_error(std::format("No CPU OP compatible op found"));
}
...
inline std::array<T *, N> generatePointerArr(std::array<RawData<>, TEN_SIZE> &ptrs) {
    ...
    for (size_t i = 0; i < N; ++i) {
        refs[i] = reinterpret_cast<T *>(ptrs[SEL_ARR[i]].data_ptr);
    }
    return refs;
}
...
inline auto exec_cpu_graph(auto &memStore) {
    ...
    if constexpr (N < EX_ARR.size() - 1) {
        exec_cpu_graph<graph, N + 1>(memStore);
    }
}
}  // namespace scions::cpu